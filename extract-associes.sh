#!/bin/bash
echo "rccm,nomEtPrenom,civilite,prenom,nom,postNom,sexe,dateNaissance,nationalite,paysNaissance,lieuNaissance,numeroPieceIdentite,adresse,email,telephoneMobile,nbPartsSociales"
jq -r "
  .details |
  {
    rccm: .[] | select(.rccm.value != null) | .rccm.value,
    person: .[] | select(.associes.value != null) | .associes.value[]
  } |
  [
    .rccm,
